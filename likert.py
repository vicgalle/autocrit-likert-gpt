...
from templates import sentiment_likert_prompt
    ...
    for _ in range(MAX_API_RETRY):
        try:
            response = openai.ChatCompletion.create(
                ...
                temperature=0.0,
                max_tokens=512,
                function_call={"name": "sentiment_likert"},
                functions=functions,
            )
...
def likert_score(prompts, outputs, prompt_template):
    ...
    for prompt, output in tqdm(zip(prompts, outputs)):
        critique_dict = get_eval(prompt_template.format(task=prompt, submission=output))
